...
| 검 | **Knife_01** (기존 에셋) | — | 이번 에셋에 칼날이 없다 |
...
| 검 애니메이션 | Mixamo 클립 확보 → Animator에 Trigger `Slash` + **새 레이어(인덱스 2)** 에 상체 마스크. 인덱스 1(ShootLayer)은 근접 무기를 들면 `WeaponController`가 끈다 |
...
| 현상 | 위치 | 원인 |
|---|---|---|
| 검을 들고 구르면 상체가 소총 자세로 돌아간다 | `PlayerMove.cs:429` | 구르기가 끝날 때 ShootLayer를 무조건 1로 되돌린다. `WeaponController`는 스왑할 때만 레이어를 조정한다 |
| Q·휠 연타로 쿨다운을 건너뛴다 | `WeaponBase.cs:112-113` | 무기를 꺼낼 때 쿨다운을 0으로 초기화한다 |
| 검이 3m 밖의 적에게도 휘두르고 쿨다운을 쓴다 | `MeleeWeapon.OnFire` | 발동 조건은 감지 반경(10m)이고 판정만 3m다. 08번 W4의 의도(사거리를 재확인한 뒤 발동)와 다르다 |
